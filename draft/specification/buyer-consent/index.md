...
On `complete_checkout`, businesses MUST NOT transition the checkout to `completed` while a confirmed consent decision has unmet data dependencies. Missing dependencies MUST be surfaced via the standard [Error Handling](https://sakinaroufid.github.io/pr-test/draft/specification/checkout/#error-handling) flow.
