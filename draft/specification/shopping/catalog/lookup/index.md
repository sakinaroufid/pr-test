...
Both `lookup_catalog` and `get_product` accept optional `filters` to narrow the returned products and variants. Filters use the same schema and AND semantics as [Search Filters](https://sakinaroufid.github.io/pr-test/draft/specification/shopping/catalog/search/#search-filters) — for example, a price filter excludes variants outside the specified range.
...
The batch Lookup response and a successful Get Product response adopt the response-only `actions` map. Batch Lookup retains its required `products` array, which can be empty; successful Get Product retains its required `product`, and its existing error response is unchanged. See [Catalog — Actions](https://sakinaroufid.github.io/pr-test/draft/specification/shopping/catalog/#actions) for the parent contract and [Overview — Actions](https://sakinaroufid.github.io/pr-test/draft/specification/overview/#actions) for the common rules.

## Transport Bindings

- [REST Binding](https://sakinaroufid.github.io/pr-test/draft/specification/shopping/catalog/rest/#post-cataloglookup): `POST /catalog/lookup` (batch)
- [REST Binding](https://sakinaroufid.github.io/pr-test/draft/specification/shopping/catalog/rest/#post-catalogproduct): `POST /catalog/product` (single)
- [MCP Binding](https://sakinaroufid.github.io/pr-test/draft/specification/shopping/catalog/mcp/#lookup_catalog): `lookup_catalog` tool (batch)
- [MCP Binding](https://sakinaroufid.github.io/pr-test/draft/specification/shopping/catalog/mcp/#get_product): `get_product` tool (single)
