...
| **Increment**                | -       | Optional [ordering granularity](https://sakinaroufid.github.io/pr-test/draft/specification/overview/#ordering-increment) of a sale basis, a count of steps; advisory for Platform-authored quantities.       |
...
| **Quantity Unit**            | -       | A [sale-basis descriptor](https://sakinaroufid.github.io/pr-test/draft/specification/overview/#quantities-and-units) with machine identity (`unit`, effective `scale`); excludes `display_text`/`increment`. |
| **Step**                     | -       | One [integer increment](https://sakinaroufid.github.io/pr-test/draft/specification/overview/#quantities-and-units) in the authoritative sale basis: `10^-scale` of `unit`.                                   |
